...
if [[ -n "$(ls -A $LIVE_CERT_FOLDER)" ]]; then
	...
	for DIR in "$LIVE_CERT_FOLDER"/*; do
		cat "$DIR/privkey.pem" "$DIR/fullchain.pem" > "$CERT_FOLDER/letsencrypt$COUNT.pem"
		(( COUNT += 1 ))
	done
fi
